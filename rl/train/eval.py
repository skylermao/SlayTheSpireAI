...
def evaluate(ckpt: str, data_path: str, n_games: int = 300, sims: int = 128,
             ...
             max_fights=None, net_kwargs=None, exclude_encounters=None,
             include_encounters=None) -> dict:
...
def main():
    ...
    ap.add_argument("--elites-only", action="store_true",
                    help="evaluate on elite encounters only")
    ...
    r = evaluate(args.ckpt, args.data, args.games, args.sims, args.concurrency,
                 ...
                 exclude_encounters=exclude, include_encounters=include)
    dt = time.time() - t0
